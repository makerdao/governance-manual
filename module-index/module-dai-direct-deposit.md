...
It is possible to deactivate the Aave D3M by setting the Target Borrow Rate to 0. Note that while intuitively setting the Target Borrow Rate to 0 might suggest that the Aave D3M will utilize it's full debt ceiling, it is a pre-programmed state in the D3M code that will de-activate the D3M entirely. It is possible to set the Target Borrow Rate to 0 using a MOM, this will bypass the [GSM Pause Delay](../parameter-index/core/param-gsm-pause-delay.md) and allow any change to take effect on execution of an Executive Vote.
